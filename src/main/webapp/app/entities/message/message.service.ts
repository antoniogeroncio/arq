...
export class MessageService {
    ...
    protected convertDateFromClient(message: IMessage): IMessage {
        const copy: IMessage = Object.assign({}, message, {
            sendDate: message.sendDate != null && message.sendDate.isValid() ? message.sendDate.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((message: IMessage) => {
                message.sendDate = message.sendDate != null ? moment(message.sendDate) : null;
            });
        }
        return res;
    }
